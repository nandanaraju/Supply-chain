...
class ProductContract extends Contract {
    ...
    async finalizeProductAtMarket(ctx, productId, marketName) {
        const mspID = ctx.clientIdentity.getMSPID();
        if (mspID === 'marketMSP') {
            const exists = await this.productExists(ctx, productId);
            if (!exists) {
            ...
            // Update product status and finalize ownership
            productDetails.status = `Finalized at Market ${marketName}`;
            productDetails.ownedBy = marketName;
    
            const updatedProductBuffer = Buffer.from(JSON.stringify(productDetails));
            await ctx.stub.putState(productId, updatedProductBuffer);
    
            return `Product ${productId} has been successfully finalized at Market ${marketName}`;
        } else {
            return `User under MSP: ${mspID} is not authorized to finalize the product at the market.`;
        }
    }
